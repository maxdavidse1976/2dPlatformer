...
public class PlayerController : MonoBehaviour
{
    ...
    public void PlayerDamaged(bool isDamageReceived)
    {
        isPlayerDamaged=isDamageReceived;
    }
    ...
    public bool grounded
    {
        get
        {
            if (groundCheck != null)
            {
                Console.WriteLine(groundCheck.CheckGrounded());
    ...
    public bool attackInput
    {
        get
        {
            if (inputManager != null)
            { 
                return inputManager.attackStarted; 
            }
            return false;
        }
    }
    ...
    [Tooltip("The duration that the player spends in the \"attack\" state")]
    public float attackDuration = 0.5f;
    [Tooltip("The effect to spawn when the player attacks")]
    public GameObject attackEffect = null;

    // Whether the player is in the middle of an attack right now
    ...
    private void SpawnAttackEffect()
    {
        ...
        {
            Instantiate(attackEffect, transform.position, transform.rotation, null);
        }
    }
    ...
    private void DetermineState()
    {
        ...
        {
            if (jumping && timesJumped <= 1 && !isPlayerDamaged)
            {
                SetState(PlayerState.Jump);
            }
            else if (jumping && timesJumped > 1 && !isPlayerDamaged)
            {
                SetState(PlayerState.Highjump);
            }
            else if (jumping && isPlayerDamaged)
            {
                SetState(PlayerState.Hurt);
