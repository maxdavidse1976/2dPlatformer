...
public class InputManager : MonoBehaviour
{
    ...
    [Header("Attack Input")]
    [Tooltip("Whether an attack was started this frame.")]
    public bool attackStarted = false;
    [Tooltip("Whether the attack button is being held.")]
    public bool attackHeld = false;

    /// <summary>
    /// Description:
    /// Reads and stores the attack input
    /// Input: 
    /// CallbackContext callbackContext
    /// Return: 
    /// void (no return)
    ...
    {
        Debug.Log("Test");
        attackStarted = !callbackContext.canceled;
        attackHeld = !callbackContext.canceled;
        if (InputManager.instance.isActiveAndEnabled)
        {
            StartCoroutine("ResetAttackStart");
        }
    }

    private IEnumerator ResetAttackStart()
    {
        yield return new WaitForEndOfFrame();
        attackStarted = false;
    }
